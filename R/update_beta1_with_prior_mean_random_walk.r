update_beta1_with_prior_mean_random_walk <- function(Xtildei,mmi,
...
xx         <- (Xtildei[ , ,1])
cc2        <- pracma::inv(pracma::inv(prior_beta_cov[ , ,1]) + (mmi * iSigmae[1,1] * (t(xx) %*% xx)))#'
mmbeta     <- nrow(beta)
...
beta1_curr <- beta[ ,1]
beta1_cand <- beta1_curr + (pracma::sqrtm(cc2/update_beta1_var_ind)$B %*% pracma::randn(mmbeta,1))
...
#gghh       = min(1,exp(lc2_cand - lc2_curr));
gghh       <- min(1,exp( lc1_cand + lc2_cand - lc1_curr - lc2_curr))

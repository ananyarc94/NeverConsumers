update_beta3_with_prior_mean <- function(Xtildei,mmi, prior_beta_mean,
...
xx         <- (Xtildei[ , ,1])
cc2        <- MASS::ginv(MASS::ginv(prior_beta_cov[ , ,3]) +
                   (mmi * iSigmae[3,3]* (t(xx) %*% xx)))#'
mmbeta     <- pracma::size(beta,1)
cc1        <- numeric(mmbeta)
cc1        <- cc1 + (MASS::ginv(prior_beta_cov[ , ,3]) %*% prior_beta_mean[ ,3])
for (jji    in 1:mmi){
    cc1    <- cc1 + (iSigmae[3,3] * (t(xx)%*% (Wtildei[ ,3,jji] - Utildei[ ,3])))
    cc1    <- cc1 + (iSigmae[1,3] * (t(xx) %*%
                    (Wtildei[ ,1,jji] - (xx %*% beta[ ,1]) - Utildei[ ,1])))
    cc1    <- cc1 + (iSigmae[2,3] * (t(xx) %*%
                    (Wtildei[ ,2,jji] - (xx %*% beta[ ,2]) - Utildei[ ,2])))
}
beta3      <- (cc2 %*% cc1) + (pracma::sqrtm(cc2)$B %*% pracma::randn(mmbeta,1))
